...
PUBLISHERS:
    goal_pose (geometry_msgs/PoseStamped) - Contains the position for the turtle robot
        to head towards
    tilt (turtle_brick_interfaces/Tilt) - Contains the angle of the platform to tilt at
    visualization_marker_ (visualization_msgs/Marker) - Contains marker of the text that
        indicates a brick is unreachable
...
class Catcher(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            ...
            2.0,
            ParameterDescriptor(
                description="The height between the turtle platform and the ground"
            ),
        )
        ...
        self.pub_marker = self.create_publisher(
            Marker, "visualization_marker_", markerQoS
        )
    ...
    def timer_callback(self):
        ...
        # If at any point, the brick's new position is larger than the old position then
        # it has been placed there/reset
        ...
        if self.state == state.FALLING:
            # And if the robot can reach it in time, move to catch it; otherwise,
            # publish marker saying it cannot
            if self.reachable:
                msg = PoseStamped()
                ...
                # Keep checking if the brick is on the platform yet,
                # and change state to CAUGHT if so
                if self.is_caught():
    ...
    def update_brick_pos(self):
        ...
        if trans_ready:
            ...
            self.brick_pos = Position3D(
                trans.transform.translation.x,
                trans.transform.translation.y,
                trans.transform.translation.z,
                0.0,
            )  # The new brick position
    ...
    def is_brick_falling(self):
        ...
        """
        if (
            self.brick_pos.z < self.old_brick_pos.z
            and self.brick_pos.x == self.old_brick_pos.x
            and self.brick_pos.y
            and self.old_brick_pos.y
        ):
            return True
        else:
            return False
    ...
    def is_caught(self):
        ...
        return self.is_near_xy(
            self.robot_pos, self.brick_pos, self.platform_radius
        ) and (self.brick_pos.z <= self.platform_height)
    ...
    def is_near_xy(self, start_pos, end_pos, rad):
        """Returns a boolean that indicates if a set of given x & y coordinates are within
            a given radius of another set of coordinates.

        Args:
            start_pos (Position): The current Position
            end_pos (Position): The target Position
            rad (float): The radius the two points have to be within of each other
                to be marked as 'near' each other
